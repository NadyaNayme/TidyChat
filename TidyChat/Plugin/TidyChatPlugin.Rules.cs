...
public sealed partial class TidyChatPlugin
{
        ...
        switch (rule.Pattern)
        {
            case PatternKind.RegexMatch:
                ...
                if (requiresCatalog && !catalogMatched && !allowTextFallback)
                {
                    if (debugMode)
                    {
                        Log.Verbose($"FAILED: {rule.Name} | LUMINA LogMessage catalog");
                    }
                    return false;
                }
                ...
                foreach (var check in rule.RegexChecks)
                {
                    if (L10N.Get(check).IsMatch(normalizedText))
                    {
                        if (debugMode)
                        {
                            Log.Debug($"MATCHED: {rule.Name} | REGEX: {L10N.Get(check)}");
                        }
                    }
                    else
                    {
            ...
            case PatternKind.StringMatch:
                ...
                foreach (var check in rule.StringChecks)
                {
                    if (L10N.Get(check).All(normalizedText.Contains))
                    {
                        if (debugMode)
                        {
                            Log.Debug($"MATCHED: {rule.Name} | CONTAINS: {string.Join(", ", L10N.Get(check))}");
                        }
                    }
                    else
                    {
    ...
    private static bool TryMatchObtainMarkerRule(LocalizedFilterRule rule, string normalizedText, bool debugMode, out bool matched)
    {
        ...
        if (rule.ObtainMarkerOtherPlayer)
        {
            ...
            {
                Log.Debug($"MATCHED: {rule.Name} | LUMINA other-player obtain marker");
            }
            return true;
        }
        ...
        if (rule.ObtainMarkerMaterials)
        {
            ...
            if (debugMode && matched)
            {
                Log.Debug($"MATCHED: {rule.Name} | LUMINA materials obtain marker");
            }
            return true;
        }
        ...
        if (rule.ObtainMarkerAnyTribal)
        {
            ...
            if (debugMode && matched)
            {
                Log.Debug($"MATCHED: {rule.Name} | LUMINA tribal currency obtain marker");
            }
            return true;
        }
        ...
        if (rule.ObtainMarkerAnySeal)
        {
            ...
            if (debugMode && matched)
            {
                Log.Debug($"MATCHED: {rule.Name} | LUMINA shared obtain + GC seal marker");
            }
            return true;
        }
        ...
        if (rule.ObtainMarkerGil)
        {
            ...
            if (debugMode && matched)
            {
                Log.Debug($"MATCHED: {rule.Name} | LUMINA shared obtain + gil marker");
            }
            return true;
        }
        ...
        if (rule.ObtainMarkerMgp)
        {
            ...
            if (debugMode && matched)
            {
                Log.Debug($"MATCHED: {rule.Name} | LUMINA shared obtain + MGP marker");
            }
            return true;
        }
        ...
        if (debugMode && matched)
        {
            Log.Debug($"MATCHED: {rule.Name} | LUMINA shared obtain + item marker");
        }
        return true;
    }
